...
app.post("/send-email", async (req, res, next) => {
  try {
    ...
    const {
      ...
      htmlBody = "<strong>Hello</strong> from the send email templates layouts tutorial.",
      textBody = "Hello from the send email templates layouts tutorial.",
    } = body;
...
const server = app.listen(PORT, HOST, () => {
  console.log(
    `Postmark send email templates layouts app listening on http://${HOST}:${PORT}`,
  );
});
